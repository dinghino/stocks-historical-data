...
def response_decorator(source, callback_generator=get_fake_response):
    ...
    def decorator(func):
        def wrapped(self_, *args, **kwargs):
            ...
            for idx, url in enumerate(get_request_urls(source)):
                responses.add_callback(responses.GET, url, callback=callback_generator(source, idx))
                response = requests.get(url, stream=True)
                func(self_, response, idx, *args, **kwargs)
