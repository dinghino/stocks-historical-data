...
def description():
    start = utils.highlight("Start date")
    end = utils.highlight("End date")
    note = utils.highlight("Currently there is no check for order", 'red')
    return (
f"""Here you can specify your date range, that will be between your {start} and
{end} dates.
{note}, so be sure to have your {start} date
set before in time than the {end} date.
"""
)
...
def handle_start_date(settings):
    ...
    utils.set_date(settings, default_date, 'start_date', "Change Start Date", description())
...
def handle_end_date(settings):
    ...
    # try:
    utils.set_date(settings, default_date, 'end_date', "Change End Date", description())
    return False
